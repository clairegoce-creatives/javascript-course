...
const calcTempAmplitudeNew = function (t1, t2) {
  ...
  console.log('Combined Array:', temps);
  ...
  for (let i = 0; i < temps.length; i++) {
    const curTemp = temps[i];
    if (typeof curTemp !== 'number') continue; // skip non-numbers
...
function demonstrateArrayMax(numbers) {
  // Method 1 - Math.max with spread operator (modern approach)
  ...
  // Method 3 - Array.reduce (functional approach)
...
function calculateAverageScore(scores) {
  ...
  return countValid === 0 ? 0 : total / countValid;
}

// Test cases
const testScores1 = [85, 92, 78, 96, 88];
console.log("Fixed result:", calculateAverageScore(testScores1)); // 87.8
console.log("Empty array ->", calculateAverageScore([])); // 0
console.log("With invalid values ->", calculateAverageScore([10, 20, "oops", 30])); // 20
